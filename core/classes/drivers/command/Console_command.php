...
class SZ_Console_command extends SZ_Command_driver
{
	...
	{
		echo 'DogFW ' . ucfirst($moduleName) . ' Testing...' . PHP_EOL;
		if ( count($files) > 0 )
		{
			foreach ( $files as $file )
			{
				...
				echo shell_exec('phpunit --colors --bootstrap ' . SZPATH . 'seezoo.php ' . $module . ' ' . SZPATH . 'tests/' . $moduleName . '/' . $module . '.php');
			}
		}
		else
		{
			echo shell_exec('phpunit --colors --bootstrap ' . SZPATH . 'seezoo.php ' . SZPATH . 'tests/' . $moduleName . '/');
		}
	}
	...
	{
		echo 'DogFW All module Testing!' . PHP_EOL;
		if ( file_exists(SZPATH . 'tests/phpunit.xml') )
		{
			echo shell_exec('phpunit --colors --bootstrap ' . SZPATH . 'seezoo.php -c tests/phpunit.xml');
		}
		else
		{
	...
	public function commandR($args)
	{
		...
		Application::get()->run(SZ_MODE_CLI, $pathInfo);
		echo PHP_EOL;
	}
}
